"""Record the B1.1 speech library with the four app voices.

Run locally (needs internet access to the Edge TTS service):
    pip install -r scripts/requirements-podcast.txt
    node scripts/extract_b1_speech_library.js
    cd scripts && python generate_b1_fixed_speech.py [--voices mia,tarek]
Writes app/assets/speech/b1-<voice>.mp3 and app/data_speech_b1.js.
"""
import asyncio
import argparse
import json
...
TEXTS_PATH = ROOT / "build" / "b1-speech-library-texts.json"
DATA_PATH = ROOT / "app" / "data_speech_b1.js"
VERSION = "b1-fixed-voices-1"
BATCH_OFFSET = 2400
...
async def generate_voice(voice, texts, concurrency):
    ...
    print(f"{voice['name']} B1.1: {len(batches)} Audiopakete")
    results = await asyncio.gather(*[
        synthesize_batch(voice, BATCH_OFFSET + index, batch, semaphore)
        for index, batch in enumerate(batches)
    ])

    voice_dir = BUILD / f"{voice['id']}-b1"
    ...
    output_path = OUTPUT / f"b1-{voice['id']}.mp3"
    subprocess.run(
        [
    ...
    if any(timing is None for timing in timings):
        raise RuntimeError(f"{voice['name']} B1.1: incomplete timing map")
    print(
        f"{voice['name']} B1.1: {duration(output_path) / 60:.1f} min, "
...
async def main():
    ...
    if DATA_PATH.exists():
        ...
        try:
            data = json.loads(source.split("window.B1_FIXED_SPEECH=", 1)[1].rsplit(";", 1)[0])
        ...
        "voices": [
            {
                ...
                "audio": f"assets/speech/b1-{voice['id']}.mp3?v={VERSION}",
    ...
    DATA_PATH.write_text(
        "/* Generated fixed cross-device B1.1 speech library. */\n"
        f"window.B1_FIXED_SPEECH={json.dumps(payload, ensure_ascii=False, separators=(',', ':'))};\n",
        encoding="utf-8",
    )

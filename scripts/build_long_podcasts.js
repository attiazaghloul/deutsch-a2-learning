...
const sceneExtensions = require('./podcast_scene_extensions');
const personas = [
  [{name:'Mia',gender:'female',age:27,voice:'de-DE-SeraphinaMultilingualNeural'}, {name:'Tarek',gender:'male',age:31,voice:'de-DE-ConradNeural'}],
  [{name:'Nora',gender:'female',age:19,voice:'de-DE-SeraphinaMultilingualNeural'}, {name:'Jonas',gender:'male',age:20,voice:'de-DE-KillianNeural'}],
  ...
  [{name:'Emma',gender:'female',age:34,voice:'de-DE-SeraphinaMultilingualNeural'}, {name:'Omar',gender:'male',age:36,voice:'de-DE-ConradNeural'}],
  ...
  [{name:'Aya',gender:'female',age:24,voice:'de-DE-SeraphinaMultilingualNeural'}, {name:'Ben',gender:'male',age:39,voice:'de-DE-KillianNeural'}],
  [{name:'Sofia',gender:'female',age:32,voice:'de-DE-SeraphinaMultilingualNeural'}, {name:'Malik',gender:'male',age:35,voice:'de-DE-ConradNeural'}],
  ...
  [{name:'Salma',gender:'female',age:26,voice:'de-DE-SeraphinaMultilingualNeural'}, {name:'David',gender:'male',age:29,voice:'de-DE-FlorianMultilingualNeural'}],
  [{name:'Hanna',gender:'female',age:51,voice:'de-DE-SeraphinaMultilingualNeural'}, {name:'Rami',gender:'male',age:38,voice:'de-DE-ConradNeural'}],
  ...
  [{name:'Nina',gender:'female',age:22,voice:'de-DE-SeraphinaMultilingualNeural'}, {name:'Adel',gender:'male',age:27,voice:'de-DE-KillianNeural'}]
];
...
function buildEpisode(chapter, shortEpisode, people) {
  ...
  (sceneExtensions[chapter.num] || []).forEach(item => {
    add(people[item[0]], item[1], item[2], .28, 'scene');
  });
  ...
  vocab.forEach((item, index) => {
    ...
    add(first, `Jetzt bist du dran. Sprich laut nach: ${word}.`, `دورك الآن. كرر الكلمة بصوت عالٍ: ${word}.`, 1.8, 'interactive');
    if ((index + 1) % 4 === 0) {
      add(second, `Mini-Wiederholung: Welche vier Wörter hast du gerade gehört? Nenne sie ohne auf den Text zu schauen.`, 'مراجعة صغيرة: ما الكلمات الأربع التي سمعتها؟ قلها من دون النظر إلى النص.', 3, 'interactive');
  ...
  phrases.forEach((item, index) => {
    const first = people[index % 2];
    const second = people[(index + 1) % 2];
    add(first, `Situation ${index + 1}, ${clean(item.cat)}. Was könntest du sagen?`, `الموقف رقم ${index + 1}. ماذا يمكنك أن تقول؟`, 2.2, 'interactive');
    add(second, `Eine passende Formulierung ist: ${clean(item.de)}`, `تعبير مناسب هو: ${clean(item.ar)}`, .7, 'phrase');
    add(first, `Wiederhole den ganzen Satz mit der gleichen Satzmelodie: ${clean(item.de)}`, `كرر الجملة كاملة بنفس نغمة الكلام.`, 2.2, 'interactive');
  });
  ...
  (chapter.quiz || []).slice(0, 4).forEach((quiz, index) => {
    ...
    add(asker, `Die Möglichkeiten sind: ${quiz.o.map((option, optionIndex) => `${String.fromCharCode(65 + optionIndex)}, ${clean(option)}`).join('. ')}.`, 'استمع إلى الاختيارات ثم أوقف الصوت وأجب.', 3.2, 'interactive');
  ...
  add(man, `Zum Abschluss: Nenne drei neue Wörter aus Kapitel ${chapter.num} und bilde mit jedem Wort einen Satz.`, `في الختام: اذكر ثلاث كلمات جديدة من الدرس ${chapter.num} وكوّن جملة بكل كلمة.`, 4, 'interactive');
